...
def generate_type_mapping(headers_list_filename = './box2d_headers_list.json', headers_dir = '../box2d_dll/box2d/include/box2d/'):
...
def generate_define_list(headers_list_filename = './box2d_headers_list.json', headers_dir = '../box2d_dll/box2d/include/box2d/', concatinate = True):
    headers_list_file = Path(headers_list_filename)
...
def _init_define_mapping(mapping_filename = './box2d_define_mapping.json'):
...
def _init_type_mapping(mapping_filename = './box2d_cindex_mapping.json'):
    mapping_file = Path(mapping_filename)
...
def register_box2d_cindex_mapping(strTypeKind, strBox2dTypedef):
    if cindex_mapping == None:
        _init_type_mapping()
    if strBox2dTypedef not in cindex_mapping:
        cindex_mapping[strBox2dTypedef] = strTypeKind

def query_box2d_cindex_mapping_entry_exists(strBox2dTypedef):
    if cindex_mapping == None:
        _init_type_mapping()
    return strBox2dTypedef in cindex_mapping

def get_box2d_cindex_mapping_value(strBox2dTypedef):
    if cindex_mapping == None:
        _init_type_mapping()
    return cindex_mapping[strBox2dTypedef]


def get_cindex_ctypes_mapping(strTypeKind, strBox2dTypedef):

    if strTypeKind == 'TypeKind.RECORD':
        return strBox2dTypedef

    isPointerToChar = False
    if strTypeKind == 'TypeKind.POINTER':
        pattern = re.compile(r"\s*char\s*\*")
        m = re.search(pattern, strBox2dTypedef)
        if m:
    ...
    if strBox2dTypedef == 'intptr_t' or strBox2dTypedef == 'uintptr_t':
    ...
    if isPointerToChar:
        ...
        return ':int64' if strBox2dTypedef == 'intptr_t' else ':uint64'
    else:
        mapping_key = ""
        if strTypeKind != 'TypeKind.TYPEDEF':
            mapping_key = strTypeKind
        else:
            mapping_key = get_box2d_cindex_mapping_value(strBox2dTypedef)

        if mapping_key == "TypeKind.RECORD":
            return str(strBox2dTypedef)
        else:
            return ctypes_mapping[mapping_key]

def is_box2d_callback_type(strTypeKind, strBox2dTypedef):
    return query_box2d_cindex_mapping_entry_exists(strBox2dTypedef) and (get_box2d_cindex_mapping_value(strBox2dTypedef) == "TypeKind.FUNCTIONPROTO")
...
class ParseContext(object):
    ...
    def __init__(self, filename = "", headers_dir = '../box2d_dll/box2d/include/box2d/'):
...
def collect_decl_typedef(ctx, cursor):
    ...
    match_obj = re.match(r"^b2(.+)", typedef_info.original_name)
    if match_obj:
...
def collect_decl_struct(ctx, cursor, struct_name=None, typedef_name=None):
    ...
    for field in cursor.get_children():
        ...
        match_obj = re.match(r"^b2(.+)", field_info.type_original_name)
        if match_obj:
            # Remove prefix 'b2' from struct name and capitalize first character
        ...
        if canonical_kind in {TypeKind.CONSTANTARRAY, TypeKind.INCOMPLETEARRAY, TypeKind.VARIABLEARRAY, TypeKind.DEPENDENTSIZEDARRAY}:
            ...
            match_obj = re.match(r"^b2(.+)", field_info.type_original_name)
            if match_obj:
                # Remove prefix b2_' from struct name and capitalize first character
                field_info.type_api_name = match_obj.group(1)[0].upper() + match_obj.group(1)[1:]
            else:
                field_info.type_api_name = field_info.type_original_name
...
def execute(ctx):
    ...
    try:
        ...
        for typedef_name, typedef_info in ctx.decl_typedefs.items():
            register_box2d_cindex_mapping(str(typedef_info.type_kind), typedef_info.api_name)
